 ...
 */
exports.visit = function (condition) {
    return function (value) {
        var init = { failures: {}, values: {} };
        if (typeof value !== 'object') {
            return exports.fail('invalid', value);
            ...
            var reports = afpl.util.reduce(value, function (r, x, k) {
                return condition(x).cata(exports.whenLeft(k, r), exports.whenRight(k, r));
            }, init);
            if (Object.keys(reports.failures).length > 0)
                return exports.mapFail(reports.failures, value);
            else
...
exports.every = function (p) {
    ...
    return function (value) {
        return p(value)
            .chain(function (b) {
            return list
                .reduce(function (e, f) { return e.chain(f); }, exports.right(b))
                .cata(function (f) { return exports.fail(f.message, value); }, function (b) { return exports.valid(b); });
        });
    };
};
/**
...
exports.range = function (min, max) {
    ...
    return function (value) {
        return (Array.isArray(value)) ?
            ((value.length < min) ? exports.fail('range.min', value, { min: min, max: max }) :
                (value.length > max) ? exports.fail('range.max', value, { min: min, max: max }) :
                    exports.valid(value)) :
            (typeof value === 'number') ?
                (value < min) ?
