...
const num = <A>(n: A) =>
    typeof n === 'number'
        ? pure(succeed<A, number>(n))
        : pure(fail<A, number>('num', n, {}));

const string = <A>(n: A) =>
    typeof n === 'string' ? pure(succeed(n)) : pure(fail('string', n, {}));
...
describe('async', function () {
    describe('filter', function () {
        it('should retain succesful members', function () {
            toPromise(filter(num)(invalidList)).then(e =>
                assert(e.takeRight()).equate([0, 1, 2, 3, 5, 7, 9])
            );
        });
    });
    ...
    describe('map', function () {
        it('should fail if any member fails', function () {
            toPromise(map(num)(invalidList)).then(e =>
                assert(e.takeLeft().explain()).equate({
                    4: 'num',
                    6: 'num',
                    8: 'num'
                })
        ...
        it('should work otherwise', function () {
            toPromise(map(num)(validList)).then(e =>
    ...
    describe('tuple', () => {
        ...
        it('should pass when valid', () => {
            ...
            return toPromise(tuple(checks)(value)).then(result => {
