...
describe('async', function () {
    describe('optional', () =>
        it('should not run the test if the value is null', () =>
            toPromise(optional(<Type>12)(undefined))
                .then(r => assert(r.takeRight()).equal(undefined))
                .then(() =>
                    toPromise(optional(() => pure(succeed(12)))('earth'))
                )
        ...
        it('should match primitives', () =>
            toPromise(
                ...
                toPromise(s('hello'))
                    ...
                    .then(r => assert(r.takeRight()).equal('Boolean'))
                    .then(() => toPromise(bcons(<Type>Date)))
                    .then(r => assert(r.takeLeft().explain()).equal('caseOf'))
            ));
    });
    ...
    describe('match', function () {
        it('should only run one case', function () {
            let p = match(
                ...
                caseOf(String, v =>
                    pure(succeed<string, string>(`${v} -> String`))
                ),
    ...
    describe('identity', function () {
        it('should return the value passed', function () {
            return toPromise(identity(12)).then(v =>
                assert(v.takeRight()).equal(12)
            );
        });
    });
    ...
    describe('discard', function () {
        it('should discard the value passed', function () {
            return toPromise(discard(12)).then(v =>
                assert(v.takeRight()).equal(undefined)
            );
        });
    });
    ...
    describe('reject', () =>
        it('should fail all the time', () =>
            toPromise(reject('testing')(12)).then(r =>
                assert(r.takeLeft().explain()).equal('testing')
            )));
